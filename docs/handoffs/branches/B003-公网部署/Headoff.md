...
- 跨分支跟进：2026-09-15，当日市场数据提交 `09173a9` 已由 B004 推送至 GitHub `main`；线上索引最新日期是否同步为 2026-09-15 尚未核验。
- 跨分支跟进：2026-09-16，当日市场数据提交 `94ca1eb` 已由 B004 推送至 GitHub `main`；线上索引最新日期是否同步为 2026-09-16 尚未核验。当日有 1 项异常与两连阴信号交集的源数据警告，应保留原始归属。
- 跨分支跟进：2026-09-17，当日市场数据提交 `e7f5386` 已由 B004 推送至 GitHub `main`；线上索引最新日期是否同步为 2026-09-17 尚未核验。
...
1. 恢复时核对 `origin/main` 是否包含 B004 的 `e7f5386` 及后续交接提交，确认 Cloudflare Pages 是否已创建、自动构建是否成功，并取得生产 URL。
2. 确认用户是否已创建 Git-integrated Pages 项目并取得生产 URL；未创建则按 `docs/DEPLOYMENT.md` 继续配置。
3. 查看 Pages 构建结果，核对生产 URL 对应的提交、`/data/index.json` 的 latest 是否为 `2026-09-17`，并检查周线筛选功能和历史日期数据提示。
